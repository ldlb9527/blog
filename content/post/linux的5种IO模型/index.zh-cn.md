...
1.在待检查文件描述符范围较小（最大文件描述符较低），或者有大量文件描述符待检查，但是其分布比较密集时poll()和select()性能相似。<br>  
...
3. 开启该套接字的信号驱动式I/O，通常通过使用fcnt的F_SETFL命令打开O_ASYNC标志完成
